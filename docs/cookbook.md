...
// GSYM splits a path into directory and basename.
...
// `Function::new` fills in the required fields; struct update syntax adds the
// optional ones.
...
fn make_builder() -> gsym::Result<GsymBuilder> {
    ...
    assert_eq!(builder.files().len(), 2); // the reserved entry plus lib.rs
...
// Owned storage can be handed back when the reader is done with it.
...
[`Gsym::for_each_frame`](crate::Gsym::for_each_frame) is the allocation-free
one: it hands each frame to a closure and reuses a caller-owned
[`LookupScratch`](crate::LookupScratch). Both accept options that switch off
record kinds you do not need.
...
// Everything resolved: name, file, line, and matching call-site patterns.
...
assert_eq!(frame.offset, 4); // 0x4004 - 0x4000
...
let cloned_builder = decoded.to_builder(options)?; // keeps `decoded` alive
assert_eq!(cloned_builder.functions().len(), 1);

let moved_builder = Gsym::parse(source.as_slice())?
    .decode_all()?
    .into_builder(options)?; // moves the trees instead of cloning
...
// Segmentation splits the model into independently readable files, each of
// which carries only the source files its own functions reference.
...
assert_eq!(GsymVersion::V2, GsymVersion::V2);
