...
pub struct Lookup<'data> {
    ...
    /// Resolved frames ordered from innermost to outermost.
    pub frames: Box<[LookupFrame<'data>]>,
    /// Call-site callee patterns for an exactly matching return address.
    pub call_site_patterns: Box<[&'data [u8]]>,
}
