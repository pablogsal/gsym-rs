...
fuzz_target!(|case: Case| {
    ...
    let target = TranscodeOptions {
        version: Some(match parsed.header().version {
            GsymVersion::V1 => GsymVersion::V2,
            GsymVersion::V2 => GsymVersion::V1,
        }),
...
fn assert_visitor_matches<D: AsRef<[u8]>>(gsym: &Gsym<D>, address: u64) {
    ...
    if let Some(expected) = expected {
        assert_eq!(frames.as_slice(), expected.frames.as_ref());
    }
}
